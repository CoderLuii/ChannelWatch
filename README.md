...
ChannelWatch v1.0.5 is a signed in-app update for the v1.0.0 runtime. Open **Settings > Updates** to install it. A v1.0.0 image running the v1.0.5 app bundle is fully current; no image pull or container recreation is required. v1.0.5 waits for a stable UI and successful Settings bootstrap before refreshing after an update, and it never exposes an empty savable form when the backend is still restarting. Installations still blocked by an abandoned pre-v1.0.2 scheduler lock can pull and recreate with the optional `1.0.5` image once while preserving `/config`.
