...
ChannelWatch v1.0.7 is a signed in-app update for the v1.0.0 runtime. Open **Settings > Updates** to install it. A v1.0.0 through v1.0.6 image running the v1.0.7 app bundle is fully current; no image pull or container recreation is required. After activation, the browser verifies the new runtime, hard-refreshes once, and opens Dashboard Overview. The first post-restart monitoring pass is shown as starting rather than stopped. Installations still blocked by an abandoned pre-v1.0.2 scheduler lock can pull and recreate with the optional `1.0.7` image once while preserving `/config`.
