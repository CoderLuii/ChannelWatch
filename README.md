...
ChannelWatch v1.0.8 is a signed in-app update for the v1.0.0 runtime. Open **Settings > Updates** to install it. A v1.0.0 through v1.0.7 image running the v1.0.8 app bundle is fully current; no image pull or container recreation is required. After activation, the browser records a durable handoff, hard-refreshes once, and opens Dashboard Overview. The dashboard then follows monitoring startup automatically instead of requiring a manual refresh. Container version and launcher compatibility come from immutable image metadata, so stale environment values cannot misidentify a recreated container. Installations still blocked by an abandoned pre-v1.0.2 scheduler lock can pull and recreate with the optional `1.0.8` image once while preserving `/config`.
