...
ChannelWatch v1.0.6 is a signed in-app update for the v1.0.0 runtime. Open **Settings > Updates** to install it. A v1.0.0 through v1.0.5 image running the v1.0.6 app bundle is fully current; no image pull or container recreation is required. v1.0.6 keeps the activated backend and browser frontend on the same app generation across UI restarts, rollback, reapply, and container restart. Installations still blocked by an abandoned pre-v1.0.2 scheduler lock can pull and recreate with the optional `1.0.6` image once while preserving `/config`.
