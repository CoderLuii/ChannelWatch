...
class CleanupMixin:
    ...
    def run_cleanup(self) -> Dict[str, Any]:
        ...
        return {"start_time": time.time(), "component": self.__class__.__name__}

    def cleanup_dict_by_time(
        self,
    ...
    def cleanup_dict_by_timestamp(self, data: Dict[Any, float], ttl: int) -> List[Any]:
    ...
    def log_cleanup_results(self, component: str, removed: Dict[str, int]) -> None:
        ...
        should_log = (
            not self.initial_cleanup_done
            or total_removed >= self.removal_threshold
            or (
                total_removed > 0
                and current_time - self.last_logged_cleanup >= self.log_cleanup_interval
            )
        )
        ...
        if should_log:
            if total_removed > 0:
                items_list = ", ".join(
                    f"{count} {item}" for item, count in removed.items() if count > 0
                )
                log(f"Cleanup: {component} - Removed {items_list}", LOG_VERBOSE)
            elif not self.initial_cleanup_done:
                log(
                    f"Cleanup: {component} initialized - nothing to clean up",
                    LOG_VERBOSE,
                )
