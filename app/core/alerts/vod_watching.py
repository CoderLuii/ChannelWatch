...
from ..helpers.parsing import is_valid_ip_address, extract_ip_address
from ..helpers.vod_info import VODInfoProvider, _metadata_id
...
class VODWatchingAlert(BaseAlert, CleanupMixin):
    ...
    ROUTING_EVENT_TYPE = "vod"
    DESCRIPTION = "Notifications when someone is watching DVR content"

    def __init__(self, alert_manager):
        """Initializes the VOD-Watching alert from an AlertManager instance."""
        ...
        self.template_settings = {
            "title": settings.vod_template_title,
            "body": settings.vod_template_body,
            "use_default": settings.vod_template_use_default,
        }
    ...
    def _should_handle_event(self, event_type: str, event_data: Dict[str, Any]) -> bool:
        ...
        is_file_event = (
            event_name.startswith("6-file-")
            or event_name.startswith("7-file")
            or (event_name.startswith("7-") and "file" in event_name)
        )
        ...
        return (not value) or (
            ("Watching" in value or "Streaming" in value) and "at" in value
        )

    async def _handle_event(self, event_type: str, event_data: Dict[str, Any]) -> bool:
        return await self._handle_event_sync(event_type, event_data)

    async def _handle_event_sync(
        ...
        try:
            ...
            if len(name_parts) >= 3:
                if name_parts[1] == "file":
                    ...
                    session_identifier = (
                        "-".join(name_parts[3:]) if len(name_parts) > 3 else ""
                    )
            ...
            if not file_id or not session_identifier:
                log(
                    f"Could not extract file ID or session identifier from event Name: {event_name}",
                    level=LOG_VERBOSE,
                )
                return False
            ...
            log(
                f"Extracted file ID: {file_id}, Identifier: {session_identifier}, SessionKey: {session_key}",
                level=LOG_VERBOSE,
            )
            log(
                f"Initial parse from Value - Device Name Candidate: {device_name_candidate}, IP Address: {ip_address_from_value}",
                level=LOG_VERBOSE,
            )
            ...
            if value:
                ...
                for existing_key, existing_data in self.active_sessions.items():
                    if (
                        existing_data.get("session_identifier") == session_identifier
                        and existing_data.get("file_id") != file_id
                    ):
                        sessions_to_remove.append(existing_key)
                        log(
                            f"DEBUG {session_key}: Detected switch. Marking old session '{existing_key}' for removal.",
                            level=LOG_VERBOSE,
                        )
                ...
                for key_to_remove in sessions_to_remove:
                    if key_to_remove in self.active_sessions:
                        ...
                        log(
                            f"DEBUG {session_key}: Removed old session '{key_to_remove}' due to VOD switch.",
                            level=LOG_VERBOSE,
                        )

            if not value:
                log(
                    f"VOD End Event: Checking for session key '{session_key}'",
                    level=LOG_VERBOSE,
                )
                session_exists = session_key in self.active_sessions
                log(
                    f"VOD End Event: Session key '{session_key}' exists? {session_exists}",
                    level=LOG_VERBOSE,
                )
                if session_exists:
                    log(
                        f"VOD End Event: Deleting session key '{session_key}'",
                        level=LOG_VERBOSE,
                    )
                    del self.active_sessions[session_key]
                else:
                    log(
                        f"VOD End Event: Session key '{session_key}' not found in active_sessions.",
                        level=LOG_VERBOSE,
                    )
                return False
            ...
            if "Streaming" in value and " at " not in value:
                log(
                    f"Streaming event detected for {session_key}, waiting for timestamped update.",
                    level=LOG_VERBOSE,
                )
            ...
            if " at " in value:
                ...
                log(
                    f"Skipping event for {session_key} due to missing timestamp in Value: {value}",
                    level=LOG_VERBOSE,
                )
                return False

            is_new_session = session_key not in self.active_sessions
            last_notification_time = self.active_sessions.get(session_key, {}).get(
                "last_notification", 0
            )
            current_time = time.time()

            if not is_new_session and (
                current_time - last_notification_time < self.alert_cooldown
            ):
                last_timestamp = self.active_sessions[session_key].get(
                    "timestamp", "0s"
                )
                log(
                    f"Cooldown active for session {session_key}. Updating timestamp: {last_timestamp} -> {current_timestamp}",
                    level=LOG_VERBOSE,
                )
                self.active_sessions[session_key].update(
                    {"timestamp": current_timestamp, "last_update": current_time}
                )
                return False
            ...
            if not is_new_session and (
                ...
                last_timestamp_str = self.active_sessions[session_key].get(
                    "timestamp", "0s"
                )
                try:
                    last_seconds = self._parse_timestamp_to_seconds(last_timestamp_str)
                    current_seconds = self._parse_timestamp_to_seconds(
                        current_timestamp
                    )
                    if (
                        abs(current_seconds - last_seconds)
                        >= self.significant_threshold
                    ):
                        log(
                            f"Significant progress detected for {session_key}. Bypassing cooldown.",
                            level=LOG_VERBOSE,
                        )
                        bypass_cooldown = True
                except ValueError:
                    log(
                        f"Could not parse timestamps for significant progress check: {last_timestamp_str}, {current_timestamp}",
                        level=LOG_VERBOSE,
                    )

            should_process_alert = (
                is_new_session
                or (current_time - last_notification_time >= self.alert_cooldown)
                or bypass_cooldown
            )

            if should_process_alert:
                log(
                    f"VOD Start/Update: Attempting to add session key '{session_key}'",
                    level=LOG_VERBOSE,
                )
                success, determined_ip_to_store = await self._process_watching_event(
                    event_data, session_key, file_id, session_identifier
                )
                ...
                if success:
                    ...
                    log(
                        f"VOD Start/Update: Successfully added session key '{session_key}'",
                        level=LOG_VERBOSE,
                    )

                    if determined_ip_to_store:
                        self.identifier_ip_cache[session_identifier] = (
                            determined_ip_to_store
                        )
                    ...
                    log(
                        f"{log_prefix} VOD session processed for alert: {session_key} at {current_timestamp}. {log_ip_storage}",
                        level=LOG_VERBOSE,
                    )
                    ...
                    log(
                        f"Processing alert for {session_key} returned False. No notification sent.",
                        level=LOG_VERBOSE,
                    )
                ...
                log(
                    f"Conditions not met to process alert for session {session_key}. Silently updating.",
                    level=LOG_VERBOSE,
                )
                return False
    ...
    async def _process_watching_event(
        ...
        try:
            ...
            if not value or " at " not in value:
                log(
                    f"Skipping processing for {session_key}: Invalid or incomplete 'Value' field (missing ' at '): {value}",
                    level=LOG_VERBOSE,
                )
                return False, None
            ...
            ip_from_id = (
                session_identifier if is_valid_ip_address(session_identifier) else ""
            )
            ...
            if ip_from_val:
                ...
                if ip_from_cache:
                    log(
                        f"DEBUG {session_key}: No IP found in current event. Using cached IP '{ip_from_cache}' for identifier '{session_identifier}'",
                        level=LOG_VERBOSE,
                    )
                    preferred_ip = ip_from_cache
                else:
                    log(
                        f"DEBUG {session_key}: No IP found in current event or cache for identifier '{session_identifier}'.",
                        level=LOG_VERBOSE,
                    )
            ...
            determined_ip_for_storage = (
                final_device_ip if final_device_ip != "Unknown IP" else None
            )

            metadata = await asyncio.to_thread(self.vod_provider.get_metadata, file_id)
            if not metadata:
                log(
                    f"No metadata found for file ID: {file_id} in session {session_key}",
                    level=LOG_VERBOSE,
                )
                return False, None

            formatted_metadata = self.vod_provider.format_metadata(
                metadata, current_time_str
            )

            if formatted_metadata.get("progress"):
                formatted_metadata["progress"] = format_timestamp(
                    formatted_metadata["progress"]
                )
            ...
            if formatted_metadata.get("duration"):
                ...
                if progress and formatted_metadata["duration"]:
                    progress_str = format_progress(
                        progress, formatted_metadata["duration"]
                    )
            ...
            if formatted_metadata.get("genres"):
                rating_genre_parts.append(
                    f"Genres: {', '.join(formatted_metadata['genres'])}"
                )
            ...
            log_device_identifier = (
                final_device_name
                if final_device_name != "Unknown Device"
                else final_device_ip
                if final_device_ip != "Unknown IP"
                else session_identifier
            )
            log(
                f"Watching {formatted_metadata.get('title', 'Unknown VOD')} - Device: {log_device_identifier}",
                level=LOG_STANDARD,
            )
            ...
            await asyncio.to_thread(
                ...
                extra={
                    ...
                    for k, v in {
                        ...
                        "episode_title": metadata.get("EpisodeTitle", ""),
            ...
            if getattr(self.settings, "alert_vod_watching", True):
                default_title = f"{self.alert_formatter.config.get('title_prefix', '')}Channels DVR - Watching DVR Content"
                rendered_alert = self.alert_formatter.format_templated_alert(
                    alert_type="vod_watching",
                    ...
                    context=self.alert_formatter.build_context(
                        ...
                        extra_context={
                            ...
                            "show_name": formatted_metadata.get("title", ""),
                            "episode_title": formatted_metadata.get(
                                "episode_title", ""
                            ),
                            ...
                            if metadata.get("SeasonNumber") not in (None, "")
                            else "",
                            "episode_number": metadata.get("EpisodeNumber", ""),
                            "episode_number00": str(
                                metadata.get("EpisodeNumber", "")
                            ).zfill(2)
                            if metadata.get("EpisodeNumber") not in (None, "")
                            else "",
                            "client_name": final_device_name,
                            "device_name": final_device_name,
                            "client_ip": final_device_ip,
                            "summary": formatted_metadata.get("summary", ""),
                            "full_summary": metadata.get("Summary", ""),
                            "image_url": formatted_metadata.get("image_url", ""),
                            "thumbnail_url": formatted_metadata.get(
                                "thumbnail_url", ""
                            ),
                            "duration": formatted_metadata.get("duration", ""),
                            "duration_secs": metadata.get("Duration", ""),
                            "playback_time": formatted_metadata.get("progress", ""),
                            "progress_percent": metadata.get("ProgressPercent", ""),
                            "content_rating": formatted_metadata.get("rating", ""),
                            "release_year": metadata.get("Year", ""),
                            "release_date": metadata.get("ReleaseDate", ""),
                            "genres": formatted_metadata.get("genres", []),
                            "cast": formatted_metadata.get("cast", []),
                            "directors": formatted_metadata.get("directors", []),
                            "tags": formatted_metadata.get("tags", []),
                            "categories": formatted_metadata.get("categories", []),
                            "watched": metadata.get("Watched", False),
                            "favorited": metadata.get("Favorite", False),
                            "library_path": metadata.get("Path", ""),
                            "channel": metadata.get("Channel", ""),
                            "media_title": message_parts[0] if message_parts else "",
                            "progress_line": progress_str
                            if formatted_metadata.get("duration")
                            and formatted_metadata.get("progress", "")
                            else "",
                            "summary_block": f"\n{formatted_metadata['summary']}\n"
                            if formatted_metadata.get("summary")
                            else "",
                ...
                alert_sent = await self.send_alert_async(
                    title=rendered_alert["title"],
                    message=rendered_alert["message"],
                    image_url=rendered_alert.get("image_url"),
                )
            else:
                alert_sent = False
    ...
    def cleanup(self):
        ...
        for identifier in stale_cached_identifiers:
            if identifier in self.identifier_ip_cache:
                ...
                log(
                    f"Cleaned up stale IP cache entry for identifier: {identifier}",
                    level=LOG_VERBOSE,
                )
