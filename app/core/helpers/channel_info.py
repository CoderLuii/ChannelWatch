...
class ChannelInfoProvider:
    ...
    def __init__(self, host=None, port=8089, cache_ttl=3600, dvr=None):
    ...
    def cache_channels(self) -> int:
        ...
        with self.cache_lock:
            ...
            if (
                self.channel_cache
                and (current_time - self.channel_cache_timestamp) < self.cache_ttl
            ):
            ...
            if response.status_code == 200:
                ...
                log(
                    f"Failed to fetch channels: HTTP {response.status_code}",
                    level=LOG_STANDARD,
                )
                return 0
