...
class JobInfoProvider:
    ...
    def __init__(
        self, host: str = "", port: int = 8089, cache_ttl: int = 86400, dvr=None
    ):
    ...
    def cache_jobs(self) -> int:
        ...
        try:
            ...
            log(
                f"Error caching jobs: Timeout occurred connecting to {url}",
                level=LOG_STANDARD,
            )
            return 0
        except httpx.RequestError as e:
            log(
                f"Error caching jobs: Network error connecting to {url}: {e}",
                level=LOG_STANDARD,
            )
            return 0
        except json.JSONDecodeError as e:
            log(
                f"Error caching jobs: Failed to decode JSON response from {url}: {e}",
                level=LOG_STANDARD,
            )
            return 0
    ...
    def get_job_by_id(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if cache_expired:
                log(
                    f"Cache expired, refreshing before job lookup for {job_id}",
                    level=LOG_VERBOSE,
                )
                self.cache_jobs()
            ...
            with self._lock:
                ...
                if job:
                    ...
                    return job
                log(
                    f"Job {job_id} not found in cache, querying API directly",
                    level=LOG_VERBOSE,
                )
            ...
            try:
                ...
                if fetch_time > 2.0:
                    log(
                        f"Slow job API call for {job_id}: {fetch_time:.2f}s",
                        level=LOG_VERBOSE,
                    )
                ...
                log(
                    f"Timeout error fetching job {job_id} (5.0s timeout exceeded)",
                    level=LOG_STANDARD,
                )
                return None
            except httpx.RequestError as e:
            ...
            log(
                f"Critical error in get_job_by_id for job {job_id}: {e}",
                level=LOG_STANDARD,
            )
            return None
    ...
    def get_recording_by_id(self, file_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            log(
                f"Error fetching recording {file_id}: Timeout occurred.",
                level=LOG_STANDARD,
            )
            return None
        except httpx.RequestError as e:
            log(
                f"Error fetching recording {file_id}: Network error: {e}",
                level=LOG_STANDARD,
            )
            return None
        except json.JSONDecodeError as e:
            log(
                f"Error fetching recording {file_id}: Failed to decode JSON response: {e}",
                level=LOG_STANDARD,
            )
            return None
        except Exception as e:
            log(
                f"Unexpected error fetching recording {file_id}: {e}",
                level=LOG_STANDARD,
            )
            return None
    ...
    def get_all_recordings(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                log(
                    f"HTTP {response.status_code} fetching all recordings",
                    level=LOG_STANDARD,
                )
            ...
            log("Error fetching all recordings: Timeout occurred.", level=LOG_STANDARD)
            return []
        except httpx.RequestError as e:
            log(
                f"Error fetching all recordings: Network error: {e}", level=LOG_STANDARD
            )
            return []
        except json.JSONDecodeError as e:
            log(
                f"Error fetching all recordings: Failed to decode JSON response: {e}",
                level=LOG_STANDARD,
            )
            return []
        except Exception as e:
