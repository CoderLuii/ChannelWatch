...
CONTROL_READ_GRACE_SECONDS = 0.25
...
class CoreNotificationDrainResponder:
    ...
    async def _read_request_with_grace(
        self,
        request_id: str,
        shutdown_event: asyncio.Event,
    ) -> tuple[str, float, int | None, float | None] | None:
        """Retry transient unreadable states without extending the lease."""

        grace_deadline = time.monotonic() + CONTROL_READ_GRACE_SECONDS
        while not shutdown_event.is_set() and time.monotonic() < grace_deadline:
    ...
    async def _wait_for_release(
        ...
        while not shutdown_event.is_set():
            ...
            if validated is None:
                validated = await self._read_request_with_grace(
                    request_id,
                    shutdown_event,
                )
            if validated is None or validated[0] != request_id:
            ...
            if next_sequence > current_sequence:
                ...
                if not lease_confirmed:
                    self._write_ack(
                        ...
                        deadline_unix=deadline_unix,
                        lease_sequence=current_sequence,
                    )
            ...
            if (
                ...
                if ack.get("status") == "drained" and time.time() < deadline_unix:
                    ...
                    confirmation_deadline = (
                        time.monotonic() + LEASE_CONFIRMATION_TIMEOUT_SECONDS
                    )
                    while time.monotonic() < confirmation_deadline:
                        held_ack = _safe_json_object(ack_path)
                        if (
                            held_ack
                            and held_ack.get("schema") == ACK_SCHEMA
                            and held_ack.get("request_id") == request_id
                            and held_ack.get("deadline_unix") == deadline_unix
                            and held_ack.get("status") == "held"
                            and type(held_ack.get("lease_sequence")) is int
                            and held_ack["lease_sequence"] >= 1
                            and renewal.is_alive()
                        ):
                            return True
                        if not renewal.is_alive():
                            break
