...
class TestNotificationManagerIntegration:
    ...
    def test_send_notification_async_failure_persists_retry_and_failed(self, engine):
        ...
        async def run():
            # Keep the production retry count while removing delays without
            # replacing the process-wide time.sleep function used by the queue
            # waiter in the other worker thread.
            with patch("core.notifications.delivery.RETRY_DELAYS", [0, 0, 0, 0, 0]):
                accepted = await nm.send_notification_async(
