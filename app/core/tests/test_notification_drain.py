...
@pytest.mark.asyncio
async def test_transient_unreadable_control_file_does_not_release_active_lease(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    responder = CoreNotificationDrainResponder(
    ...
    def transient_read(_path):
        nonlocal reads
        reads += 1
        return None if reads == 1 else valid_request
    ...
    waiter = asyncio.create_task(
        responder._wait_for_release(
            request_id,
            valid_request["deadline_unix"],
            0,
            1.0,
            0,
            shutdown,
...
def test_initial_drain_ack_without_confirmed_hold_cannot_authorize_update(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setattr(
        notification_drain_module,
        "LEASE_CONFIRMATION_TIMEOUT_SECONDS",
        0.1,
    )
    ...
    def acknowledge_only_initial_drain():
        deadline = time.monotonic() + 1.0
        while time.monotonic() < deadline:
            request = notification_drain_module._safe_json_object(
                runtime / notification_drain_module.REQUEST_FILE
            )
            if request:
                responder._write_ack(
    ...
    fake_core = threading.Thread(target=acknowledge_only_initial_drain)
    fake_core.start()
    try:
        assert (
            request_core_notification_drain(
        ...
        fake_core.join(timeout=1.0)

    assert fake_core.is_alive() is False
