...
describe("Security section auth flows", () => {
  ...
  it("keeps legacy key migration and destructive reset on authenticated endpoints", () => {
    const section = srcFile("../components/settings/security-section.tsx")
    const api = srcFile("../lib/api.ts")

    expect(section).toContain('data-testid="key-recovery-card"')
    expect(section).toContain('const KEY_RESET_CONFIRMATION = "RESET PROTECTED CREDENTIALS"')
    expect(section).toContain('confirmation !== KEY_RESET_CONFIRMATION')
    expect(section).toContain('type="password"')
    expect(section).toContain('type="file"')
    expect(section).toContain("clearMigrationInputs()")
    expect(section).toContain("RUNTIME_PREFLIGHT_CHANGED_EVENT")
    expect(section).toContain("window.dispatchEvent(new CustomEvent(RUNTIME_PREFLIGHT_CHANGED_EVENT))")
    expect(section).toContain("wrappingInputRef.current.value = \"\"")
    expect(section).not.toContain("localStorage")
    expect(section).not.toContain("sessionStorage")
    expect(section).not.toContain("CHANNELWATCH_SECRET_STORAGE_KEY:")
    expect(api).toContain("/v1/runtime/key-recovery/status")
    expect(api).toContain("/v1/runtime/key-recovery/migrate")
    expect(api).toContain("/v1/runtime/key-recovery/reset")
    expect(api).toContain('body.set("legacy_storage_key"')
    expect(api).toContain('body.set("raw_key_file"')
