...
self.onmessage = (event) => {
  const rawDeadlineMs = event.data.deadlineMs
  const rawYieldEvery = event.data.yieldEvery
  const deadlineMs = Number.isFinite(rawDeadlineMs) && rawDeadlineMs >= 0 ? rawDeadlineMs : 30000
  const yieldEvery = Number.isInteger(rawYieldEvery) && rawYieldEvery > 0 ? rawYieldEvery : 256
  ...
  try {
    ...
    for (let attempt = 0; attempt < Number.MAX_SAFE_INTEGER; attempt += 1) {
      ...
      if (hasLeadingZeroBits(sha256(encoder.encode(`${challenge.nonce}.${solution}`)), challenge.difficulty)) {
        self.postMessage({ type: "success", proof: { ...challenge, solution } })
        return
      }
      if ((attempt + 1) % yieldEvery === 0) {
        ...
        if (now - started >= deadlineMs) {
          throw new Error("Secure report preparation timed out. Your report and attachments are still here; retry, use the upload portal, or download the offline package.")
        }
      }
    }
    ...
    self.postMessage({
      type: "error",
      message: error instanceof Error ? error.message : "Secure report preparation failed.",
    })
  }
}
