...
- `1.0.6` for a fresh install containing the current v1.0.6 app; existing operational v1.0.0 through v1.0.5 installations update in-app
...
Existing operational v1.0.0 through v1.0.5 installations install v1.0.6 through **Settings > Updates**. A compatible v1.0 container running the signed v1.0.6 bundle is fully current and does not need to be recreated. v1.0.6 keeps the activated backend and browser frontend on the same app generation. If an older Update Center is already trapped by an abandoned scheduler lock, preserve `/config` and recreate with `coderluii/channelwatch:1.0.6` once; no settings or credentials need to be re-entered.
