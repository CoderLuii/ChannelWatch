...
- `1.0.5` for a fresh install containing the current v1.0.5 app; existing operational v1.0.0 through v1.0.4 installations update in-app
...
Existing operational v1.0.0 through v1.0.4 installations install v1.0.5 through **Settings > Updates**. A v1.0.0 container running the signed v1.0.5 bundle is fully current and does not need to be recreated. v1.0.5 waits for a stable Settings bootstrap before refreshing the browser. If an older Update Center is already trapped by an abandoned scheduler lock, preserve `/config` and recreate with `coderluii/channelwatch:1.0.5` once; no settings or credentials need to be re-entered.
