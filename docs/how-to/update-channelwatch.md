...
ChannelWatch v1.0.7 is a signed in-app release on that line. An operational v1.0.0 through v1.0.6 installation discovers and installs it through **Settings > Updates**. After activation, `Application version` shows v1.0.7 while `Container image version` may remain an earlier v1.0 image. That combination is fully current and does not require an image refresh. The updater waits for two stable target-runtime and Settings checks, changes the route to Dashboard Overview, and hard-refreshes the activated frontend once. During the first monitoring pass after Core restarts, the dashboard reports monitoring as starting rather than falsely saying it stopped.
...
ChannelWatch versions that include the reconnect improvement wait for the target runtime, verify Settings twice, open Dashboard Overview, and hard-refresh the page automatically. Rejected, failed, or timed-out applies remain on the Update Center so their remediation stays visible. Refresh manually only if browser code older than this improvement cannot reconnect. Direct v0.9.11–v0.9.17 upgrades to v0.9.18 use a compatible launcher and preserve `/config`; v0.9.9 and v0.9.10 instead require the one-time image pull/recreate described above.
...
Documentation, Compose, Helm, or Unraid presentation changes alone do not make an otherwise ABI-compatible app bundle image-required. v1.0.7 still publishes normal AMD64/ARM64 images for fresh installations and lock-recovery cases, but operational v1.0.0 through v1.0.6 users do not need to pull them.
