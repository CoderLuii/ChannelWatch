...
| Key contents | Logical 32-byte random key generated and managed by ChannelWatch under persistent `/config`. Legacy envelopes created by v0.9.5–v0.9.17 can be opened once with their old deployment input and normalized without changing the logical key. |
...
The key is generated atomically before protected settings are written. No user-generated deployment key is required. Backup and restore code treats `encryption.key` as sensitive because it is required to decrypt per-DVR API keys; protect `/config` and app backup archives like credentials.
