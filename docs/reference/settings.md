...
- `rd_image` | JSON path: `rd_image` | Type: boolean | Default: `true`, including existing installations | Description: Include program artwork in recording notifications. Turn off under Settings > Alerts > Recording Outcomes > Program Image; recording cards and activity history still show artwork. | Valid values: `true`, `false`. | Env override: none. | Reload behavior: `restart-required`.
